"""
飞书消息发送技能 - 通过飞书机器人Webhook发送消息

A skill for sending messages via Feishu (Lark) bot webhook.
"""
...
class FeishuMessageSkill(BaseSkill):
    """通过飞书机器人Webhook发送消息的技能"""

    def get_name(self) -> str:
        return "feishu_message"

    def get_description(self) -> str:
        return "通过飞书（Lark）机器人的Webhook发送消息到飞书群聊或私聊。支持发送文本消息、富文本消息、卡片消息等。"
    ...
    def get_parameters(self) -> Dict[str, Any]:
        return {
            ...
            "properties": {
                "webhook_url": {
                    "type": "string",
                    "description": "飞书机器人Webhook URL，用于发送消息的地址",
                },
                "message": {
                    "type": "string",
                    "description": "要发送的消息内容，支持文本消息和JSON格式的卡片消息",
                },
                "msg_type": {
                    "type": "string",
                    "description": "消息类型，支持 text、interactive、post 等类型，默认为 text",
                    "default": "text"
                }
            },
    ...
    def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200 and response_data.get("code") == 0:
                ...
                return {
                    "success": False,
                    "message": f"消息发送失败: {response_data.get('msg', '未知错误')}",
                    "response": {
                        "status_code": response.status_code,
                        "data": response_data
                    }
